...
function template_page($id, $flag = TEMPLATE_DISPLAY) {
	...
	$compile = IA_ROOT . "/data/tpl/".APP_NAME."/{$id}.{$_W['template']}.tpl.php";
	...
	$content .= "<script type=\"text/javascript\" src=\"./resource/js/".APP_NAME."/common.js\"></script>";
...
function template($filename, $flag = TEMPLATE_DISPLAY,$data=[]) {
	global $_W, $_GPC;
    if($data){
        extract($data);
    }
	//从根目录的模版目录中找模版
	$source = ROOT_PATH.TEMPLATE_PATH .THEME."/".APP_NAME."/{$filename}.html";
	$compile = DATA_PATH . "/tpl/".THEME."/".APP_NAME."/{$filename}.php";
	if(!is_file($source)) {
	...
	//从项目中的模版目录中找模版
	if(!is_file($source)) {
		$source = APP_PATH .TEMPLATE_PATH.THEME."/{$filename}.html";
		$compile = DATA_PATH . "tpl/".THEME."/".APP_NAME."/{$filename}.php";
	}
	//找系统默认的模版
    if(!is_file($source)) {
        $source = CORE_PATH .TEMPLATE_PATH.THEME."/{$filename}.html";
        $compile = DATA_PATH . "tpl/".THEME."/{$filename}.php";
    }
	if(!is_file($source)) {
		exit("Error: template source '{$filename}' is not exist! {$source}");
	}
	$paths = pathinfo($compile);
	$compile = str_replace($paths['filename'], THEME . '_' . intval($_GPC['t']) . '_' . $paths['filename'], $compile);
...
function template_parse($str) {
	...
	$str = "<?php defined('ACCESS') or exit('Access Denied');?>" . $str;
	return $str;
}
