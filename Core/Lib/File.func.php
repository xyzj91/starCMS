...
if(!function_exists("file_upload")) {
    function file_upload($file, $type = 'image', $name = '')
    {
        ...
        if (empty($name) || $name == 'auto') {
            ...
            mkdirs(ROOT_DIR . '/' . $path);
            $filename = file_random_name(ROOT_DIR . '/' . $path, $ext);

            $result['path'] = $path . $filename;
        } else {
            mkdirs(dirname(ROOT_DIR . '/' . $name));
        ...
        if (!file_move($file['tmp_name'], ROOT_DIR . '/' . $result['path'])) {
...
if(!function_exists("file_wechat_upload")) {
...
if(!function_exists("file_remote_upload")) {
    ...
    function file_remote_upload($filename, $auto_delete_local = true)
    {
        ...
        if ($_W['setting']['remote']['type'] == '1') {
            ...
            if (true === $ftp->connect()) {
                $response = $ftp->upload(ROOT_DIR . '/' . $filename, $filename);
            ...
            $options = array(
                ALIOSS::OSS_FILE_UPLOAD => ROOT_DIR . $filename,
            ...
            list($ret, $err) = $uploadmgr->putFile($uploadtoken, $filename, ROOT_DIR . '/' . $filename);
            if ($auto_delete_local) {
                file_delete($filename);
            }
            if ($err !== null) {
    ...
    function file_image_thumb($srcfile, $desfile = '', $width = 0)
    {
        ...
        if ($org_info) {
            if ($width == 0 || $width > $org_info[0]) {
                ...
                return str_replace(ROOT_DIR . '/', '', $desfile);
            }
            if ($org_info[2] == 1) {
...
if(!function_exists("file_image_crop")) {
